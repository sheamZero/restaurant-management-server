...
async function run() {
    try {
        ...
        app.patch("/menu/:id", verifyToken, verifyUserEmail, verifyAdmin, async (req, res) => {
            const id = req.params.id;
        ...
        app.post("/menu", verifyToken, verifyUserEmail, verifyAdmin, async (req, res) => {
            try {
        ...
        app.delete("/menu/:id", verifyToken, verifyUserEmail, verifyAdmin, async (req, res) => {
            const id = req.params.id;
        ...
        app.get("/payment-history", verifyToken, verifyUserEmail, async (req, res) => {
            try {
                const email = req.verifiedEmail;
                const query = { email };
        ...
        app.post("/payments", verifyToken, verifyUserEmail, async (req, res) => {
            try {
                ...
                if (type === "reservation" && reservationIds.length > 0) {
                    ...
                    updatedReservationResult = await reservationCollections.updateMany(query, {
                ...
                res.send({ paymentResult, deletedCartResult, updatedReservationResult, emailSent: true });
            }
            catch (err) {
        ...
        app.get("/dashboard/reservation", verifyToken, verifyUserEmail, async (req, res) => {
            try {
                const email = req.verifiedEmail;
        ...
        app.post("/dashboard/reservation", verifyToken, async (req, res) => {
            try {
                ...
                if (email) {
                    ...
                    const result = await reservationCollections.insertOne(reserve_details);
                    res.send(result);
                }
                else {
        ...
        app.patch("/admin/reservation/:id", async (req, res) => {
            ...
            try {
                const result = await reservationCollections.updateOne(filter, updatedDoc);
                res.send(result);
            } catch (err) {
                res.status(500).send({ error: err.message })
            }
        ...
        app.delete("/dashboard/reservation/:id", verifyToken, verifyUserEmail, async (req, res) => {
            try {
                const u_id = req.params.id;
        ...
        // users related apis
        app.get("/admin/users/:email", verifyToken, verifyUserEmail, async (req, res) => {
        ...
        app.patch("/users/:id", verifyToken, verifyUserEmail, verifyAdmin, async (req, res) => {
            const u_id = req.params.id;
            const query = { _id: new ObjectId(u_id) };
            const updatedDoc = {
                $set: {
        ...
        app.delete("/users/:id", verifyToken, verifyUserEmail, verifyAdmin, async (req, res) => {
            try {
                const u_id = req.params.id;
                const query = { _id: new ObjectId(u_id) };
